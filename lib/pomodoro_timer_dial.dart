...
class PomodoroTimerDial extends StatefulWidget {
  ...
  PomodoroTimerDial({
    ...
    this.maxTime = const Duration(minutes: 25),
  ...
  @override
  _PomodoroTimerDialState createState() => new _PomodoroTimerDialState();
}

class _PomodoroTimerDialState extends State<PomodoroTimerDial> with TickerProviderStateMixin {
...
class _DialTurnGestureDetectorState extends State<DialTurnGestureDetector> {
  ...
  _onRadialDragUpdate(PolarCoord coord) {
    if (startDragCoord != null) {
      ...
      angleDiff = angleDiff >= 0.0 ? angleDiff : angleDiff + (2 * math.pi);
      ...
      print('Novo tempo: ${selectedTime.inMinutes}');
